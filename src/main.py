...
def main():
    ...
    if args.train:
        ...
        model, name = vgg_model(num_classes) if args.mode == "vgg" \
            else chinese_model(num_classes)
        model.load_state_dict(torch.load(f'trained_models/{args.model}_{args.dataset}.weights'))
        prune_model(model, data_loaders)
        torch.save(model.state_dict(), f'trained_models/pruned_{args.model}_{args.dataset}.weights')
